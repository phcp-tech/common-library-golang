...
//
// Note: GORM's MySQL driver pings the server during Open — Init() returns a
// non-nil error immediately when the database is unreachable.
func loadFromEnv() error {
	...
	if err := mysqlInitDefault(config); err != nil {
...
// Init calls loadFromEnv; Close closes the default GORM database connection.
//
// mysqlInitDefault is the InitDefault implementation used by loadFromEnv.
// It is a package-level variable so that tests can replace it with a stub.
var mysqlInitDefault = mysql.InitDefault

// GORM's MySQL driver pings the server on Open — Init() returns a non-nil error
...
func Component() bootstrap.IComponent {
	return bootstrap.Func(
		...
		func() {
			...
			if err := dbgorm.Close(db); err != nil {
