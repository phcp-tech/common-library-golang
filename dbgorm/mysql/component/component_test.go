...
func TestMain(m *testing.M) {
	if err := env.InitEnv("testdata/config.toml"); err != nil {
		panic("dbgorm/mysql/component tests: failed to load testdata/config.toml: " + err.Error())
	}
	os.Exit(m.Run())
}
...
//
// GORM's MySQL driver pings the server during Open — port 13307 (testdata config)
// always refuses connections, so Init() returns a non-nil error immediately.
// dbgorm.Default() remains nil because InitDefault only calls SetDefault on success.
...
//
// Because Init() always fails with the testdata config (port 13307 unreachable),
...
// mysqlInitDefault is stubbed to return nil so the success branch is reached
// without requiring a live MySQL server.
func TestLoadFromEnv_Success(t *testing.T) {
	prev := mysqlInitDefault
	mysqlInitDefault = func(_ *mysql.Config) error { return nil }
	defer func() { mysqlInitDefault = prev }()
...
// when Init() always fails due to an unreachable MySQL host.
