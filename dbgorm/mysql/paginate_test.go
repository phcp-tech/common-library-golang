...
func TestZhSortSql(t *testing.T) {
	tests := []struct {
		...
		{
			name:          "unsafe sort falls back to plain id sort, no CONVERT",
		...
		{
			name:          "unsafe sort with an otherwise-valid charset still skips CONVERT",
