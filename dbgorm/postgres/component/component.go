...
//
// Note: GORM's PostgreSQL driver pings the server and runs SHOW search_path on
// Open — Init() returns a non-nil error immediately when the database is unreachable.
...
// GORM's PostgreSQL driver pings the server on Open — if the database is
