...
import (
	"github.com/phcp-tech/common-library-golang/dbgorm"
...
// or ClickHouse. dbgorm.SortSql deliberately does not offer this — see its
// doc comment for why. For MySQL use dbgorm/mysql.ChineseSortSql instead.
//
...
func ChineseSortSql(para *dto.PageParameter) string {
	...
	if !dbgorm.IsSafeSQLIdentifierPath(para.Sort) || !dbgorm.IsSafeSQLName(charset) {
		return dbgorm.SortSql(para)
	}

	para.Direction = dbgorm.NormalizeSortDirection(para.Direction)
