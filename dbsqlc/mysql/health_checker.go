...
// HealthChecker returns a [health.Checker] that verifies MySQL connectivity
// by pinging the default connection.
// The returned Checker reports [health.StatusUnhealthy] when no connection has been
...
func HealthChecker() health.Checker {
	return func(ctx context.Context) health.Result {
		...
		if Default() != nil {
			if err := Default().PingContext(ctx); err == nil {
