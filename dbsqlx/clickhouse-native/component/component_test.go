...
func TestMain(m *testing.M) {
	if err := env.InitEnv("testdata/config.toml"); err != nil {
		panic("dbsqlx/clickhouse-native/component tests: failed to load testdata/config.toml: " + err.Error())
	}
	os.Exit(m.Run())
}
...
// TestComponent_Name verifies the component name is "clickhouse-native".
func TestComponent_Name(t *testing.T) {
	if c := Component(); c.Name() != "clickhouse-native" {
		t.Errorf("Component().Name() = %q, want %q", c.Name(), "clickhouse-native")
	}
}
...
// clickhousenativeInitDefault is stubbed to return an error.
func TestLoadFromEnv_Error(t *testing.T) {
	prev := clickhousenativeInitDefault
	clickhousenativeInitDefault = func(_ *clickhousenative.Config) error {
		return errors.New("stub: connect failed")
	}
	defer func() { clickhousenativeInitDefault = prev }()
...
// clickhousenativeInitDefault is stubbed to return nil.
func TestLoadFromEnv_Success(t *testing.T) {
	prev := clickhousenativeInitDefault
	clickhousenativeInitDefault = func(_ *clickhousenative.Config) error { return nil }
	defer func() { clickhousenativeInitDefault = prev }()
...
// After this test, clickhousenative.Default() is non-nil (singleton set by InitDefault).
...
//
// Because Init() succeeds with a lazy driver, clickhousenative.Default() is non-nil,
