...
// package clickhousenative_test demonstrates the public API from a caller's perspective.
package clickhousenative_test
...
// Zero-value pool fields fall back to the dbsqlx package defaults
...
func ExampleNewClickHouse() {
	conn, err := clickhousenative.NewClickHouse(&clickhousenative.Config{
		Host:     "127.0.0.1",
		Port:     "9000",
		Database: "default",
		Username: "default",
...
// After a successful call, Default returns the initialised driver.Conn ready
// for direct use.
func ExampleInitDefault() {
	if err := clickhousenative.InitDefault(&clickhousenative.Config{
		Host:     "127.0.0.1",
		Port:     "9000",
		Database: "default",
		Username: "default",
	...
	_ = clickhousenative.Default() // driver.Conn, ready to use
}
...
func ExampleHealthChecker() {
	results := health.Check(context.Background(), clickhousenative.HealthChecker())
