...
func TestMain(m *testing.M) {
	if err := env.InitEnv("testdata/config.toml"); err != nil {
		panic("dbsqlx/mysql/component tests: failed to load testdata/config.toml: " + err.Error())
	}
	os.Exit(m.Run())
}
...
//
// Open verifies connectivity with an eager ping — port 13307 (testdata config)
