...
func TestMain(m *testing.M) {
	if err := env.InitEnv("testdata/config.toml"); err != nil {
		panic("dbsqlx/sqlite/component tests: failed to load testdata/config.toml: " + err.Error())
	}
	os.Exit(m.Run())
}
...
//
// Because Init() succeeds with the in-memory path, dbsqlx.Default() is non-nil,
// so Close() exercises the full dbsqlx.Close() and slog.Info path.
func TestComponent_Close_AfterInit(t *testing.T) {
	c := Component()
	_ = c.Init() // no-op if already set; dbsqlx.Default() is non-nil
	c.Close()    // must not panic; exercises dbsqlx.Close + slog.Info path
}
...
// dbsqlx.Default() is set to nil so the close function exits early.
