...
func TestMain(m *testing.M) {
	if err := env.InitEnv("testdata/config.toml"); err != nil {
		panic("redis/component tests: failed to load testdata/config.toml: " + err.Error())
	}
	os.Exit(m.Run())
}
...
func TestComponent_Name(t *testing.T) {
	c := Component()
	if c.Name() != "redis" {
		t.Errorf("Component().Name() = %q, want %q", c.Name(), "redis")
	}
}
...
// so Init() always succeeds regardless of server reachability.
// redis.InitDefault uses sync.Once — this is the sole Init call in this binary.
